...
// 纯函数：模型输出与代码校验分离，"một dòng có phải tiêu đề chương hay không"不由 Go 复判，但覆盖不变量必须成立。
func resolveSegmentation(normalized []byte, units []SourceUnit, decisions []BoundaryDecision) (*Segmentation, error) {
	...
	for i := 1; i < len(units); i++ {
		if !unitLess(units[i-1], units[i]) {
			return nil, fmt.Errorf("SourceUnit chưa sắp theo thứ tự số (Line,Part): %s đứng trước %s", units[i-1].ID, units[i].ID)
		}
	}
	...
	for i, d := range decisions {
		...
		default:
			return nil, fmt.Errorf("boundary[%d] kind không hợp lệ: %q", i, d.Kind)
		}
	...
	for _, p := range points {
		if n := len(uniq); n > 0 && uniq[n-1].byte == p.byte {
			...
			if prev := uniq[n-1].d; prev.Kind != p.d.Kind || boundaryLabel(prev) != boundaryLabel(p.d) {
				notes = append(notes, fmt.Sprintf("Ranh giới %q và %q trùng nhau (byte %d), đã giữ mục trước",
	...
	if head := points[0].byte; head != 0 && strings.TrimSpace(string(normalized[:head])) != "" {
		notes = append(notes, fmt.Sprintf("%d byte đầu chưa được model gán (%s…), đã gom vào front_matter; hãy kiểm tra có bỏ sót chương không",
	...
	for i, p := range points {
		...
		case kindChapter:
			if strings.TrimSpace(bodyAfterTitle(normalized, p.byte, end)) == "" {
				// 真实网络小说源常见"chương đã khóa/trả phí"占位：标题在、正文缺失。不整体失败——
				// 终局一票否决会浪费切分阶段的全部模型调用；标题行并入前段（文本一字不丢），
				// 记入 Notes 由确认预览呈现，人工不认可可用 --guide 裁定（RFC §8.4 的停点正为此存在）。
				seg.Notes = append(seg.Notes,
					fmt.Sprintf("Tiêu đề chương %q không có nội dung (byte %d..%d), đã nhập vào đoạn trước (thường gặp ở chương placeholder bị khóa/trả phí)", title, start, end))
				if !absorb(end) {
	...
	if chapterNo == 0 {
		return nil, fmt.Errorf("Cắt đoạn không tạo ra chương nào (group không tính là chương)")
	}
	// 同名章节是"cùng chương bị cắt nhầm"的确定性信号（有标题规约的源里章名不该重复），只记 Notes
	...
	for _, c := range seg.Chapters {
		...
		if first, ok := titleAt[key]; ok && key != "" {
			seg.Notes = append(seg.Notes, fmt.Sprintf("Chương %d và chương %d có tiêu đề giống nhau (%q), nghi cùng chương bị cắt nhầm; hãy kiểm tra",
...
// 此时返回全段而非空串——否则合法的单行/单行多章小说会被误判"nội dung rỗng"拒绝（RFC §8.3）。
...
func Segment(ctx context.Context, m callModel, systemPrompt string, normalized []byte, units []SourceUnit, guidance string, chunkBytes, contextMargin, maxTokens int, prof callProfile, w *Workspace, identity string) (*Segmentation, error) {
	...
	chunk = func(owned [2]int, cur, total int) ([]BoundaryDecision, error) {
		...
		prof.step(cur, total, "Cắt đoạn khối %d/%d (%s..%s), đã nhận diện %d ranh giới...",
		...
		if err != nil {
			...
			if errors.As(err, &tr) && owned[1]-owned[0] > 1 {
				...
				prof.logger().Warn("imp đầu ra cắt đoạn bị cắt, chia đôi khối", "chunk", lo.ID+".."+hi.ID)
			...
			return nil, fmt.Errorf("cắt đoạn khoảng %s..%s: %w", lo.ID, hi.ID, err)
		}
		...
		if n := len(batch.Boundaries) - len(kept); n > 0 {
			...
			prof.step(0, 0, "Đã loại %d ranh giới báo dư trong vùng ngữ cảnh (để khối lân cận tự báo, không phải lỗi)", n)
		}
		// 回显模型的语义判断（识别出的标题），让用户看见模型读懂了什么，而非只有机械计数。
		if len(kept) > 0 {
			prof.step(0, 0, "Model nhận diện: %s", previewBoundaries(kept))
		}
		if w != nil {
			if err := writeArtifact(w, rel, want, boundaryBatch{Boundaries: kept}); err != nil {
				return nil, fmt.Errorf("lưu khối cắt đoạn %s..%s: %w", lo.ID, hi.ID, err)
			}
		}
		return kept, nil
	}
	...
	if err != nil {
		...
		hint := "Cache khối đã được xóa, chạy lại sẽ cắt đoạn lại"
		if w != nil {
		...
		return nil, &errSemantic{Raw: string(raw), Err: fmt.Errorf("Tổng hợp cắt đoạn toàn truyện thất bại (%s): %w", hint, err)}
	}
	return seg, nil
}
...
func (v chunkValidator) validate(bs []BoundaryDecision) error {
	...
	for _, b := range bs {
		if b.UnitID == "" {
			return fmt.Errorf("boundary thiếu unit_id")
		}
		if !v.projIDs[b.UnitID] {
			return fmt.Errorf("boundary unit_id %q không tồn tại trong projection lần này", b.UnitID)
		}
		...
		case kindChapter, kindGroup, kindFrontMatter, kindBackMatter:
		default:
			return fmt.Errorf("boundary %s kind không hợp lệ: %q (chỉ có thể là chapter/group/front_matter/back_matter)", b.UnitID, b.Kind)
		}
		...
		if (b.Kind == kindChapter || b.Kind == kindGroup) && !b.Uncertain {
			if t := squashSpace(b.Title); t != "" && !strings.Contains(squashSpace(v.unitByID[b.UnitID].Text), t) {
				return fmt.Errorf("Không tìm thấy tiêu đề %q của boundary %s trong nguyên văn unit: nếu đây là phần tiếp nội dung của chương trước, đừng đặt boundary cho nó (để boundary trước gán, boundaries có thể rỗng); nếu nguồn thật sự không có dòng tiêu đề và tiêu đề là bạn quy nạp, hãy đặt uncertain=true",
		...
		if prev, ok := seen[at]; ok {
			if prev.Kind != b.Kind || boundaryLabel(prev) != boundaryLabel(b) {
				return fmt.Errorf("Boundary %q và %q nằm cùng vị trí (%s), xung đột ngữ nghĩa; chỉ giữ một boundary đúng",
	...
	if v.coverStart {
		...
		if head > 0 && strings.TrimSpace(string(v.normalized[:head])) != "" {
			return fmt.Errorf("%d byte đầu (%s…) chưa thuộc boundary nào; hãy bổ sung boundary ở đầu văn bản (front_matter/chapter/group)",
