...
func Run(ctx context.Context, deps Deps, opts Options) (<-chan Event, error) {
	...
	log.Info("imp runtime model nhập",
	...
	go func() {
		...
		if logErr != nil {
			r.emit(StageIngesting, 0, 0, fmt.Sprintf("Tạo file log nhập thất bại (%v), bản ghi lần này chuyển sang log mặc định", logErr), nil)
		}
		r.run(ctx)
	}()
	return events, nil
...
func (r *runner) applyGuidance() error {
	...
	if err != nil {
		return fmt.Errorf("đọc hướng dẫn cắt đoạn hiện có: %w", err)
	}
	if existing == g {
		return nil
	}
	...
	if p != "" {
		return fmt.Errorf("Foundation chính thức đã bắt đầu được xuất bản; cắt đoạn lại bằng --guide sẽ xung đột với nội dung đã xuất bản và bị từ chối ghi đè, nên không nhận hướng dẫn cắt đoạn nữa")
	}
...
func (r *runner) checkSourceIdentity() error {
	...
	if Digest(raw) != m.RawSHA256 {
		return fmt.Errorf("Đang có lượt nhập %q; file nguồn lần này khác nội dung. Hãy hoàn tất hoặc bỏ lượt nhập cũ trước (xóa meta/import/) rồi nhập sách mới", m.SourceName)
	}
	return nil
}
...
func (r *runner) run(ctx context.Context) {
	...
	for {
		...
		if err := r.applyGuidance(); err != nil {
			r.fail("Ghi hướng dẫn cắt đoạn", err)
			return
		}
		facts, err := r.facts()
		if err != nil {
		...
		if previous != nil && facts == *previous {
			r.fail("Nhập bị kẹt", fmt.Errorf("sau khi thực thi hành động, dữ kiện không đổi; hành động tiếp theo vẫn là %q", NextAction(facts)))
			return
		}
		snapshot := facts
		previous = &snapshot
		...
		case ActionPublish:
			err = r.publish(ctx)
		case ActionDone:
			r.emit(StageDone, 0, 0, "Nhập xong, chờ nghiệm thu rồi viết tiếp", nil)
			return
		default:
			err = fmt.Errorf("hành động chưa biết %q", act)
		}
		if err != nil {
...
func (r *runner) ingest(ctx context.Context) error {
	...
	if r.ws.Active() {
		return fmt.Errorf("meta/import/ đã tồn tại nhưng danh tính workspace không dùng được (manifest/source/intent thiếu hoặc hỏng); hãy kiểm tra thủ công rồi xóa thư mục này trước khi nhập lại")
	}
	if err := checkImportPreconditions(r.deps.Store); err != nil {
		return err
	}
	if r.opts.SourcePath == "" {
		return fmt.Errorf("Lượt nhập mới cần đường dẫn file nguồn")
	}
	r.emit(StageIngesting, 0, 0, "Đọc, giải mã, chuẩn hóa và chụp snapshot file nguồn...", nil)
	_, m, err := Ingest(r.deps.Store.Dir(), r.opts.SourcePath, r.opts.intent())
	if err != nil {
		return err
	}
	r.emit(StageIngesting, 0, 0, fmt.Sprintf("Snapshot nguồn đã sẵn sàng: %s (mã hóa %s, %d byte)", m.SourceName, m.Encoding, m.SizeBytes), nil)
	return nil
}
...
func (r *runner) segment(ctx context.Context) error {
	...
	if err != nil {
		return fmt.Errorf("đọc hướng dẫn cắt đoạn: %w", err)
	}
	...
	if cerr := r.ws.clearDir(dirSegmentChunks); cerr != nil {
		r.emit(StageSegmenting, 0, 0, fmt.Sprintf("Dọn cache cấp khối thất bại (không ảnh hưởng kết quả cắt đoạn): %v", cerr), nil)
	}
	r.emit(StageSegmenting, len(seg.Chapters), len(seg.Chapters),
		fmt.Sprintf("Cắt đoạn xong: %d chương, %d vùng phụ trợ", len(seg.Chapters), len(seg.Matter)), nil)
	return nil
}
...
func (r *runner) confirm() bool {
	...
	if err != nil {
		r.fail("Đọc kết quả cắt đoạn", err)
		return false
	}
	in, err := r.ws.LoadIntent()
	if err != nil {
		r.fail("Đọc intent nhập", err)
		return false
	}
	...
	if !auto && !accept {
		...
		if blockedByNotes {
			msg += "  ! Có ghi chú dung sai cắt đoạn, --yes chưa tự động cho qua; hãy kiểm tra thủ công\n"
		}
	...
	if err != nil {
		r.fail("Đọc artifact cắt đoạn", err)
		return false
	}
	method, doneMsg := confirmMethodAuto, "Đã tự động chấp nhận cắt đoạn (--yes)"
	if accept {
		method, doneMsg = confirmMethodUser, "Đã xác nhận cắt đoạn (kiểm tra thủ công)"
	}
...
func buildConfirmPreview(seg *Segmentation) string {
	...
	fmt.Fprintf(&b, "Đã cắt %d chương", len(seg.Chapters))
	if len(seg.Matter) > 0 {
...
func (r *runner) analyze(ctx context.Context) error {
	...
	for {
		...
		r.emit(StageAnalyzing, start, total, fmt.Sprintf("Phân tích batch liên tục bắt đầu từ chương %d...", start+1), nil)
	...
	r.emit(StageAnalyzing, total, total, "Trích xuất dữ kiện từng chương xong", nil)
	return nil
}
...
func (r *runner) synthesize(ctx context.Context) error {
	...
	if len(facts) != total {
		return fmt.Errorf("phân tích từng chương chưa hoàn chỉnh: %d/%d", len(facts), total)
	}
	r.emit(StageSynthesizing, 0, total, "Quy nạp ngữ nghĩa toàn truyện theo tầng...", nil)
...
func (r *runner) publish(ctx context.Context) error {
	...
	facts := loadPriorFacts(r.ws, total)
	if len(facts) != total {
		return fmt.Errorf("phân tích trước xuất bản chưa hoàn chỉnh: %d/%d", len(facts), total)
	}
	...
	r.emit(StageValidating, 0, total, "Kiểm tra lắp ghép Foundation đã qua", nil)

	r.emit(StagePublishing, 0, total, "Xuất bản Foundation chính thức...", nil)
	...
	if err := r.setCompletionHold(); err != nil {
		return fmt.Errorf("tạo Hold hoàn tất nhập: %w", err)
	}
	for i, c := range seg.Chapters {
		if ctx.Err() != nil {
			return ctx.Err()
		}
		r.emit(StagePublishing, c.Number, total, fmt.Sprintf("Xuất bản chương %d/%d: %s", c.Number, total, c.Title), nil)
...
func (r *runner) storyChoice() (string, error) {
	...
	in, err := r.ws.LoadIntent()
	if err != nil {
		return "", fmt.Errorf("Đọc intent nhập: %w", err)
	}
...
func (r *runner) resolveStoryStatus() bool {
	...
	if choice != storyOpen && choice != storyClosed {
		r.emit(StageAwaitingStoryStatus, 0, 0, "Tổng hợp phán định story_status là uncertain; hãy dùng --story=open|closed để làm rõ rồi thử lại", nil)
		return false
	}
	...
	if err := writeArtifact(r.ws, fileStoryResolve, Digest(raw), StoryResolution{Choice: choice}); err != nil {
		r.fail("Lưu phán định trạng thái truyện", err)
		return false
	}
	return true
}
...
func (r *runner) resolveStory(syn *BookSynthesis) (bool, error) {
	...
	case storyUncertain:
		...
		case storyOpen:
			return false, nil
		default:
			return false, fmt.Errorf("story_status là uncertain, cần --story=open|closed")
		}
	default:
		return false, fmt.Errorf("story_status chưa biết: %q", syn.StoryStatus)
	}
}
...
func (r *runner) setCompletionHold() error {
	...
	return r.deps.Store.RunMeta.SetAdvanceHold(domain.AdvanceHold{
		...
		Reason: "Nhập tiểu thuyết bên ngoài xong, chờ nghiệm thu rồi viết tiếp",
	})
}
