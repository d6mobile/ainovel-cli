...
func LoadState(store *storepkg.Store) (State, error) {
	...
	if err != nil {
		return s, fmt.Errorf("tải run meta: %w", err)
	}
	if meta != nil {
	...
	if err != nil {
		return s, fmt.Errorf("tải progress: %w", err)
	}
	if progress == nil {
		return s, nil
	}
	...
	if progress.Layered && s.LastCompleted > 0 {
		...
		if err != nil {
			return s, fmt.Errorf("kiểm tra ranh giới cung: %w", err)
		}
		if boundary != nil {
			...
			if boundary.IsArcEnd {
				...
				if err != nil {
					return s, fmt.Errorf("tải review cung: %w", err)
				}
	...
	if !progress.Layered && s.LastCompleted > 0 {
		if due, _ := domain.ShouldReview(len(progress.CompletedChapters)); due {
			...
			if err != nil {
				return s, fmt.Errorf("tải review toàn cục: %w", err)
			}
		}
	}
