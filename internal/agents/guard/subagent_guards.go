...
func newCheckpointDeltaGuard(st *store.Store, agentName string, requiredSteps []string, blockMsg func(seen map[string]struct{}) string, onBlock BlockHook) agentcore.StopGuard {
	...
	return func(_ context.Context, info agentcore.StopInfo) agentcore.StopDecision {
		...
		if n > subagentMaxConsecutiveBlocks {
			slog.Error("subagent stop_guard chặn liên tiếp quá giới hạn, nâng cấp thành kết thúc",
...
func writerBlockMsg(seen map[string]struct{}) string {
	...
	case !hasDraft && !hasEdit:
		return "Cấm kết thúc: lượt này chưa lưu bất kỳ nội dung chính văn nào. Hãy hoàn thành chương theo thứ tự plan_chapter → draft_chapter → check_consistency → commit_chapter; chỉ xuất chính văn trong chat đồng nghĩa với mất dữ liệu, bắt buộc phải lưu bằng công cụ rồi submit."
	case !hasCheck:
		return "Cấm kết thúc: chính văn đã được lưu nhưng chưa chốt. Hãy gọi check_consistency để kiểm tra nhất quán trước, rồi gọi commit_chapter để submit chương này. draft_chapter / edit_chapter chỉ lưu bản nháp, chưa tính là hoàn thành."
	default:
		return "Cấm kết thúc: chương này chỉ còn thiếu commit_chapter. Hãy gọi commit_chapter ngay; nếu nó trả về lỗi, xử lý theo thông tin lỗi trước (kiểm tra số chương, bổ sung bước tiền đề theo gợi ý) rồi thử submit lại, không kết thúc khi chưa submit."
	}
}
...
func NewArchitectStopGuard(st *store.Store, onBlock BlockHook) agentcore.StopGuard {
	return newCheckpointDeltaGuard(st, "architect",
		...
		staticBlockMsg("Bạn phải gọi save_foundation để lưu sản phẩm trước khi có thể kết thúc. Chỉ xuất chữ Markdown/JSON đồng nghĩa với mất dữ liệu."),
		onBlock,
	)
}
...
func NewEditorStopGuard(st *store.Store, task string, onBlock BlockHook) agentcore.StopGuard {
	...
	case strings.Contains(task, "save_volume_summary") || strings.Contains(task, "卷摘要"):
		return newCheckpointDeltaGuard(st, "editor", []string{"volume_summary"},
			staticBlockMsg("Nhiệm vụ lần này là tạo tóm tắt tập: bạn phải gọi save_volume_summary để lưu trước khi có thể kết thúc; save_review chỉ là đánh giá lại, chưa tính là hoàn thành."), onBlock)
	case strings.Contains(task, "save_arc_summary") || strings.Contains(task, "弧摘要"):
		return newCheckpointDeltaGuard(st, "editor", []string{"arc_summary"},
			staticBlockMsg("Nhiệm vụ lần này là tạo tóm tắt cung: bạn phải gọi save_arc_summary để lưu trước khi có thể kết thúc; save_review chỉ là đánh giá lại, chưa tính là hoàn thành."), onBlock)
	default:
		// 评审或临时任务：任一审阅/摘要落盘即可（保持既有宽松行为）。
		return newCheckpointDeltaGuard(st, "editor",
			[]string{"review", "arc_summary", "volume_summary"},
			staticBlockMsg("Bạn phải gọi một trong save_review / save_arc_summary / save_volume_summary để lưu kết quả trước khi có thể kết thúc."), onBlock)
	}
}
